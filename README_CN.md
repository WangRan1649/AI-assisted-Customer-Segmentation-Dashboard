...
## V3.4 Eval Harness 评估框架

V3.4 新增轻量 Eval Harness，用于测试 Router Agent 和 Orchestrator 是否稳定工作。它会检查 intent 识别、tool selection、风险等级、dry-run 安全边界、高风险请求拒绝，以及回答中是否包含关键业务信息。

Eval 是本地确定性测试，不会调用 SiliconFlow，不会切换 raw 数据，不会运行 pipeline，也不会修改 Power BI 文件。
...
页面更适合 HR 展示、中文面试复盘和作品集讲解：业务解释、AI 洞察、页面说明和 Agent 回答以中文为主；字段名、文件名、命令、`Provider`、`Intent`、`Tools`、`Risk Level`、`Dry Run`、`Trace` 等系统信息保留英文，方便展示工程边界。

页面采用咨询公司风格布局：白底、深蓝 / 墨蓝主色、细灰色分割线、紧凑卡片和高信息密度 overview cards。页面支持自然语言工作流提问、从 `data/processed/customer_segments.csv` 读取 RFM 分群摘要、预览 Power BI 输出文件、查看 Eval 结果，并通过 sidebar 按钮显式触发 demo raw 切换、pipeline 运行和 eval 运行。

页面默认使用 `mock` provider。它不会自动切换 raw 数据、不会自动运行 pipeline，也不会默认调用真实 API。会修改本地工作流输出的动作，都必须由用户点击按钮触发。
